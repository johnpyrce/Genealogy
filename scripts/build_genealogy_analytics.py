...
from scripts.lib.genealogy_data import ROOT, REGISTRIES


DEFAULT_DATABASE = ROOT / "artifacts" / "analytics" / "genealogy_analytics.duckdb"
DEFAULT_OUTPUT = ROOT / "artifacts" / "analytics" / "genealogy_analytics.json"
SCHEMA_VERSION = 1
...
def create_tables(connection: duckdb.DuckDBPyConnection) -> None:
    ...
    csv_files = {
        "people": REGISTRIES / "people.csv",
        "families": REGISTRIES / "families.csv",
        "sources": REGISTRIES / "sources.csv",
        "relationship_evidence": REGISTRIES / "relationship_evidence.csv",
    }
...
def supplemental_queries() -> dict[str, str]:
    ...
    return {
        ...
        "oldest_people": """
            SELECT
                TRIM(CONCAT_WS(' ', first_name, surname)) AS person_name,
                birth_year,
                death_year,
                note
            FROM people
            WHERE birth_year IS NOT NULL
            ORDER BY birth_year, first_name, surname, id
            LIMIT 10
        """,
    }
